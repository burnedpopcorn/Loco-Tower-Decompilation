function load_model(argument0, argument1) //load_model
{
    ...
    for (var i = 0; i < n; i++)
    {
        ...
        switch type
        {
            ...
            case 2:
                ...
                var yy = line[2]
                var zz = line[3]
                vertex_add_point(model, xx, yy, zz, 0, 0, 0, 16777215, 1, 0, 0)
                break
            case 3:
                xx = line[1]
                yy = line[2]
            ...
            case 4:
                ...
                var xtex = line[4]
                var ytex = line[5]
                vertex_add_point(model, xx, yy, zz, 0, 0, 0, 16777215, 1, xtex, ytex)
                break
            case 5:
                xx = line[1]
                yy = line[2]
            ...
            case 6:
                ...
                var ny = line[5]
                var nz = line[6]
                vertex_add_point(model, xx, yy, zz, nx, ny, nz, 16777215, 1, 0, 0)
                break
            case 7:
                xx = line[1]
                yy = line[2]
...
function load_obj(argument0, argument1, argument2) //load_obj
{
    ...
    {
        line = file_text_read_string(mtl_file)
    ...
    {
        line = file_text_read_string(obj_file)
        ...
        switch terms[0]
        {
            ...
            case "f":
                for (n = 1; n <= 3; n++)
                {
                    ...
                    for (i = 1; i <= string_length(terms[n]); i++)
                    {
                    ...
                    xx = ds_list_find_value(vertex_x, (real(data[0]) - 1))
                    yy = ds_list_find_value(vertex_y, (real(data[0]) - 1))
                    zz = ds_list_find_value(vertex_z, (real(data[0]) - 1))
                    xtex = ds_list_find_value(vertex_xtex, (real(data[1]) - 1))
                    ytex = ds_list_find_value(vertex_ytex, (real(data[1]) - 1))
                    nx = ds_list_find_value(vertex_nx, (real(data[2]) - 1))
                    ny = ds_list_find_value(vertex_ny, (real(data[2]) - 1))
                    nz = ds_list_find_value(vertex_nz, (real(data[2]) - 1))
                    color = 16777215
                    alpha = 1
