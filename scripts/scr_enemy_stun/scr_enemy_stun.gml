function scr_enemy_stun() //scr_enemy_stun
{
    ...
    if (hitvsp < 0 && place_meeting_solid(x, (y - 1)) && (!(place_meeting(x, (y - 1), obj_destructibles))))
    {
        if (thrown == 1)
        {
            if (((!elite) || elitehit <= 0) && destroyable)
    ...
    if (hithsp != 0 && place_meeting_solid((x + hithsp), y) && (!(place_meeting_slope((x + sign(hithsp)), y))) && (!(place_meeting((x + hithsp), y, obj_destructibles))) && (!(place_meeting((x + hsp), y, obj_enemyblock))))
    {
        if (thrown == 1)
        {
            if (((!elite) || elitehit <= 0) && destroyable)
