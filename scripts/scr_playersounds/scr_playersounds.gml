function scr_playersounds() //scr_playersounds
{
    with (obj_player1)
    {
        if (sprite_index == spr_mach1 && state == 70 && (!audio_is_playing(sfx_mach1)))
        {
            mach1snd = audio_play_sound(sfx_mach1, 1, false)
            sfx_gain(mach1snd)
        }
        else if (sprite_index != spr_mach1 || state != 70)
            audio_stop_sound(mach1snd)
        if ((sprite_index == spr_mach || state == 17) && (!audio_is_playing(sfx_mach2)))
        {
            mach2snd = audio_play_sound(sfx_mach2, 1, false)
            sfx_gain(mach2snd)
        }
        else if (sprite_index != spr_mach && state != 17)
            audio_stop_sound(mach2snd)
        if (sprite_index == spr_machroll && state == 2 && (!audio_is_playing(sfx_machroll)))
        {
            machrollsnd = audio_play_sound(sfx_machroll, 1, false)
            sfx_gain(machrollsnd)
        }
        else if (sprite_index != spr_machroll || state != 2)
            audio_stop_sound(machrollsnd)
        if (sprite_index == spr_mach2jump && state == 70 && (!audio_is_playing(sfx_mach2jump)))
        {
            mach2jumpsnd = audio_play_sound(sfx_mach2jump, 1, false)
            sfx_gain(mach2jumpsnd)
        }
        else if (sprite_index != spr_mach2jump || state != 70)
            audio_stop_sound(mach2jumpsnd)
        if (sprite_index == spr_dive && state == 2 && (!audio_is_playing(sfx_dive)))
        {
            divesnd = audio_play_sound(sfx_dive, 1, false)
            sfx_gain(divesnd)
        }
        else if (sprite_index != spr_dive || state != 2)
            audio_stop_sound(divesnd)
        if ((state == 91 || sprite_index == spr_mach3boost) && sprite_index != spr_crazyrun && (!audio_is_playing(sfx_mach3)))
        {
            mach3snd = audio_play_sound(sfx_mach3, 1, false)
            sfx_gain(mach3snd)
        }
        else if ((state != 91 && sprite_index != spr_mach3boost) || sprite_index == spr_crazyrun)
            audio_stop_sound(mach3snd)
        if (state == 74 && freefallsmash >= 1 && (!audio_is_playing(sfx_groundpoundloop)))
        {
            groundpoundloopsnd = audio_play_sound(sfx_groundpoundloop, 1, false)
            sfx_gain(groundpoundloopsnd)
        }
        else if (state != 74 || freefallsmash <= 0)
            audio_stop_sound(groundpoundloopsnd)
        if (state == 18 && (!audio_is_playing(sfx_knightslide)))
        {
            knightslide = audio_play_sound(sfx_knightslide, 1, false)
            sfx_gain(knightslide)
        }
        else if (state != 18 && audio_is_playing(sfx_knightslide))
            audio_stop_sound(knightslide)
        if ((sprite_index == spr_bombpeprun || sprite_index == spr_bombpeprunabouttoexplode) && (!audio_is_playing(sfx_bombpep1)))
        {
            bombpep1snd = audio_play_sound(sfx_bombpep1, 1, false)
            sfx_gain(bombpep1snd)
        }
        else if (sprite_index != spr_bombpeprun && sprite_index != spr_bombpeprunabouttoexplode && audio_is_playing(sfx_bombpep1))
            audio_stop_sound(bombpep1snd)
        if (sprite_index == spr_crazyrun && (!audio_is_playing(sfx_mach4)))
        {
            mach4snd = audio_play_sound(sfx_mach4, 1, false)
            sfx_gain(mach4snd)
        }
        else if (sprite_index != spr_crazyrun)
            audio_stop_sound(mach4snd)
        if (state != 65 && audio_is_playing(superjumpprepsnd))
            audio_stop_sound(superjumpprepsnd)
        if (state != 65 && audio_is_playing(superjumpholdsnd))
            audio_stop_sound(superjumpholdsnd)
        if (sprite_index == spr_tumblestart && (!audio_is_playing(sfx_tumble1)) && floor(image_index) < 11)
        {
            tumble1snd = audio_play_sound(sfx_tumble1, 1, false)
            sfx_gain(tumble1snd)
        }
        if (sprite_index == spr_tumblestart && floor(image_index) == 11 && (!audio_is_playing(sfx_tumble2)))
        {
            tumble2snd = audio_play_sound(sfx_tumble2, 1, false)
            audio_stop_sound(tumble1snd)
            sfx_gain(tumble2snd)
        }
        if (sprite_index == spr_tumble && (!audio_is_playing(sfx_tumble3)))
        {
            tumble3snd = audio_play_sound(sfx_tumble3, 1, false)
            sfx_gain(tumble3snd)
        }
        if (state != 2 && sprite_index != spr_machroll)
        {
            audio_stop_sound(tumble1snd)
            audio_stop_sound(tumble2snd)
            audio_stop_sound(tumble3snd)
        }
        if (audio_is_playing(suplexdashsnd) && state != 22)
            audio_stop_sound(suplexdashsnd)
    }
    with (obj_player2)
    {
        if (state == 69 && (!audio_is_playing(sfx_mach1)) && grounded)
            mach1snd = audio_play_sound(sfx_mach1, 1, false)
        else if (state != 69 || (!grounded) || move == (-xscale))
            audio_stop_sound(mach1snd)
        if ((sprite_index == spr_mach || state == 17) && (!audio_is_playing(sfx_mach2)))
            mach2snd = audio_play_sound(sfx_mach2, 1, false)
        else if (sprite_index != spr_mach && state != 17)
            audio_stop_sound(mach2snd)
        if ((state == 91 || sprite_index == spr_mach3boost) && sprite_index != spr_crazyrun && (!audio_is_playing(sfx_mach3)))
            mach3snd = audio_play_sound(sfx_mach3, 1, false)
        else if ((state != 91 && sprite_index != spr_mach3boost) || sprite_index == spr_crazyrun)
            audio_stop_sound(mach3snd)
        if (state == 18 && (!audio_is_playing(sfx_knightslide)))
            knightslide = audio_play_sound(sfx_knightslide, 1, false)
        else if (state != 18 && audio_is_playing(sfx_knightslide))
            audio_stop_sound(knightslide)
        if ((sprite_index == spr_bombpeprun || sprite_index == spr_bombpeprunabouttoexplode) && (!audio_is_playing(sfx_bombpep1)))
            bombpep1snd = audio_play_sound(sfx_bombpep1, 1, false)
        else if (sprite_index != spr_bombpeprun && sprite_index != spr_bombpeprunabouttoexplode && audio_is_playing(sfx_bombpep1))
            audio_stop_sound(bombpep1snd)
        if (sprite_index == spr_crazyrun && (!audio_is_playing(sfx_mach4)))
            mach4snd = audio_play_sound(sfx_mach4, 1, false)
        else if (sprite_index != spr_crazyrun)
            audio_stop_sound(mach4snd)
        if (state != 65 && superjumpprepsnd)
            audio_stop_sound(superjumpprepsnd)
        if (state != 65 && audio_is_playing(superjumpholdsnd))
            audio_stop_sound(superjumpholdsnd)
        if (sprite_index == spr_tumblestart && (!audio_is_playing(sfx_tumble1)) && floor(image_index) < 11)
            tumble1snd = audio_play_sound(sfx_tumble1, 1, false)
        if (sprite_index == spr_tumblestart && floor(image_index) == 11 && (!audio_is_playing(sfx_tumble2)))
        {
            tumble2snd = audio_play_sound(sfx_tumble2, 1, false)
            audio_stop_sound(tumble1snd)
        }
        if ((sprite_index == spr_tumble || sprite_index == spr_machroll) && (!audio_is_playing(sfx_tumble3)))
            tumble3snd = audio_play_sound(sfx_tumble3, 1, false)
        if (state != 2 && sprite_index != spr_machroll)
        {
            audio_stop_sound(tumble1snd)
            audio_stop_sound(tumble2snd)
            audio_stop_sound(tumble3snd)
        }
        if (audio_is_playing(suplexdashsnd) && state != 22)
            audio_stop_sound(suplexdashsnd)
    }
}

