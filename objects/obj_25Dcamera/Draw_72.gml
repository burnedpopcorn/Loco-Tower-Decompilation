...
if (target.state == 70 || target.state == 91 || target.state == 2 || target.state == 44)
{
    var _targetcharge = (target.xscale * ((target.movespeed / 4) * 50))
    var _tspeed = 0.3
    chargecamera = Approach(chargecamera, _targetcharge, _tspeed)
}
else if (abs(target.hsp) >= 16 && target.state != 17 && target.state != 63)
{
    _targetcharge = (target.xscale * ((abs(target.movespeed) / 4) * 50))
    _tspeed = 2
    if ((_targetcharge > 0 && chargecamera < 0) || (_targetcharge < 0 && chargecamera > 0))
        _tspeed = 8
    chargecamera = Approach(chargecamera, _targetcharge, _tspeed)
}
else if (target.state == 71)
