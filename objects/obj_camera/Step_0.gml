...
if ((!instance_exists(obj_ghostcollectibles)) && room != Special_hall)
{
...
if ((instance_exists(obj_ghostcollectibles) || room == Special_hall) && alarm[1] > 0 && keepsecondalarm != alarm[1])
{
...
if (room != rm_2point5test && room != hall_treasureV2)
{
    ...
    if (instance_exists(player) && player.state != 36 && player.state != 55)
    {
        if (player != obj_playerspecialstage || (player == obj_playerspecialstage && player.state != 6))
        {
            ...
            {
                if (player.state == 70 || player.state == 91 || player.state == 2 || player.state == 44)
                {
                    var _targetcharge = (player.xscale * ((player.movespeed / 4) * 50))
                    var _tspeed = 0.3
                    chargecamera = Approach(chargecamera, _targetcharge, _tspeed)
                }
                else if (abs(player.hsp) >= 16 && player.state != 17 && player.state != 63)
                {
